...
export async function GET(request) {
  ...
  try {
    ...
    return NextResponse.json({
      users: users.map((u) => ({ id: u.id, name: u.name, admin: u.admin, ativo: u.ativo })),
    });
  } catch (err) {
...
export async function POST(request) {
  ...
  const { name, password, admin } = await request.json().catch(() => ({}));
  ...
  try {
    ...
    const created = await createAppUser(name.trim(), hash, { admin: !!admin });
