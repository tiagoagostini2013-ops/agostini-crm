...
export function flattenItem(item) {
  ...
  const propostas =
    propostasRaw && Array.isArray(propostasRaw.files)
      ? propostasRaw.files.map((f) => ({
          fileId: String(f.fileId),
          name: f.name,
          isAsset: f.fileType === 'ASSET',
          // url é preenchido depois (app/api/items/route.js), via consulta
          // separada assets(ids: ...) — o value bruto da coluna não traz URL.
          url: null,
        }))
      : [];
