...
export function flattenItem(item) {
  ...
  // Vendedor de pós-venda (secundário do handoff) — coluna "people" própria,
  // limitada a 1 pessoa, separada da coluna "Responsável" (que continua com
  // os dois). Ver nota em lib/config.js sobre por que isso existe.
  const vendedorPosVendaRaw = getRaw('vendedorPosVenda');
  // Nome do campo igual dos dois lados (mesmo padrão de "responsavelIds"):
  // o que a PATCH recebe em `fields.vendedorPosVenda` é o mesmo nome que o
  // item devolve pro front-end — assim onSaved()/updateLocalItem() em
  // Dashboard.jsx refletem a mudança na hora, sem precisar recarregar.
  const vendedorPosVenda =
    vendedorPosVendaRaw && vendedorPosVendaRaw.personsAndTeams && vendedorPosVendaRaw.personsAndTeams[0]
      ? String(vendedorPosVendaRaw.personsAndTeams[0].id)
      : null;

  const whatsappRaw = getRaw('whatsapp');

  // Cliente cadastrado retroativamente pelo botão "+ Novo Pós-venda" (ver
  // lib/config.js) — mesmo padrão de leitura de checkbox já usado em
  // lib/users.js (admin/ativo): o value bruto da coluna é {checked: true|false}.
  const clienteHistorico = !!getRaw('clienteHistorico')?.checked;
  ...
  // Vendas recorrentes registradas pra este cliente (ex: reposição de
  // peças) — mesmo padrão de JSON-em-long_text de "contatos" (ver
  // lib/config.js). Escrito só pela rota dedicada
  // app/api/items/[id]/vendas-adicionais/route.js (leitura-modificação-
  // escrita, mesmo princípio das anotações), nunca pelo PATCH genérico.
  let vendasAdicionais = [];
  const vendasAdicionaisText = get('vendasAdicionais');
  if (vendasAdicionaisText) {
    try {
      const parsed = JSON.parse(vendasAdicionaisText);
      if (Array.isArray(parsed)) vendasAdicionais = parsed;
    } catch {
      vendasAdicionais = [];
    }
  }

  // Histórico de envios de proposta em PDF com rastreio de leitura (ver
  // lib/config.js) — mesmo padrão de JSON-em-long_text de "contatos".
  let rastreioPropostas = [];
  const rastreioText = get('rastreioPropostas');
  if (rastreioText) {
    try {
      const parsed = JSON.parse(rastreioText);
      if (Array.isArray(parsed)) rastreioPropostas = parsed;
    } catch {
      rastreioPropostas = [];
    }
  }
  ...
  return {
    ...
    vendedorPosVenda,
    estagioPosVenda: get('estagioPosVenda'),
    propostas,
    contatos,
    dataQualificacao: get('dataQualificacao'),
    dataFechamento: get('dataFechamento'),
    dataPerda: get('dataPerda'),
    dataPrimeiroContato: get('dataPrimeiroContato'),
    dataPropostaEnviada: get('dataPropostaEnviada'),
    dataNegociacao: get('dataNegociacao'),
    dataEntregaEfetiva: get('dataEntregaEfetiva'),
    vendasAdicionais,
    rastreioPropostas,
    clienteHistorico,
  };
...
export function buildColumnValues(fields) {
  ...
  // Vendedor de pós-venda — mesma forma de "pessoas", mas só 1 pessoa (ou
  // nenhuma, se algum dia precisar desfazer um handoff). Escrito
  // automaticamente pelo handoff (ver HandoffModal/LeadDrawer), nunca por um
  // campo de formulário livre.
  if (fields.vendedorPosVenda !== undefined) {
    columnValues[COLUMNS.vendedorPosVenda] = fields.vendedorPosVenda
      ? { personsAndTeams: [{ id: Number(fields.vendedorPosVenda), kind: 'person' }] }
      : { personsAndTeams: [] };
  }

  if (fields.estagioPosVenda !== undefined) {
    columnValues[COLUMNS.estagioPosVenda] = fields.estagioPosVenda ? { label: fields.estagioPosVenda } : null;
  }

  // Marca o cliente como cadastrado retroativamente (ver lib/config.js) —
  // escrito só na criação, pelo NovoPosVendaModal, nunca editável depois.
  if (fields.clienteHistorico !== undefined) {
    columnValues[COLUMNS.clienteHistorico] = { checked: fields.clienteHistorico ? 'true' : 'false' };
  }
