...
export const COLUMNS = {
  ...
  // Passagem de bastão estruturada — criadas em 02/09/2026 a pedido do
  // Tiago, depois de ele notar que o card do Kanban de vendas ficava
  // "bagunçado" com dois vendedores listados juntos depois do handoff.
  // Antes disso, o handoff só ADICIONAVA o vendedor secundário à mesma
  // coluna de Responsável (pessoas) — sem nenhum jeito estruturado de saber
  // quem era o principal e quem era o secundário (só existia em texto livre
  // na anotação). Essas duas colunas resolvem isso: `vendedorPosVenda`
  // guarda só o secundário (people, limitada a 1 pessoa) e `estagioPosVenda`
  // guarda o estágio dele no ciclo de vida pós-entrega — ver o Kanban
  // arrastável da aba Pós-venda (PosVenda.jsx). São preenchidas
  // automaticamente pelo handoff (ver HandoffModal/LeadDrawer), nunca
  // manualmente. `pessoas` continua tendo os dois (principal + secundário)
  // — isso não mudou, é usado pra permissões/filtros/histórico — só a forma
  // de EXIBIR passou a diferenciar os dois papéis.
  vendedorPosVenda: 'multiple_person_mm6t5nnt', // Vendedor Pós-venda — people (máx. 1)
  estagioPosVenda: 'color_mm6txvjr', // Estágio Pós-venda — status
  // "Fechado" no funil de vendas = venda assinada, não máquina entregue —
  // o Tiago notou isso em 04/09/2026 depois de usar o Kanban de Pós-venda
  // uma semana: o handoff (que continua acontecendo no fechamento, pra o
  // pós-venda já se apresentar) não deveria marcar a entrega como já feita.
  // Esta coluna guarda a data real de entrega, carimbada só quando alguém
  // arrasta o card pra "Entregue" no Kanban de Pós-venda — nunca no
  // fechamento em si. Ver POS_VENDA_STAGE_DATE_COLUMNS abaixo.
  dataEntregaEfetiva: 'date_mm6we43n', // Data de Entrega Efetiva — date
  // Vendas recorrentes pra um cliente já fechado (ex: reposição de peças) —
  // pedido do Tiago em 08/09/2026, usando o Lucas Andrade como exemplo. Ele
  // confirmou que esse tipo de venda costuma ser direta (liga, combina,
  // confirma — sem passar pela proposta/negociação formal do funil de
  // vendas) e que NÃO deveria virar um card novo no Kanban principal — só um
  // registro vinculado ao cliente, na aba Pós-venda. Por isso isto é uma
  // lista (JSON, mesmo padrão de "Contatos e Decisores") direto na coluna
  // do próprio cliente, não um item novo no board.
  vendasAdicionais: 'long_text_mm70vwe2', // Vendas Adicionais — long_text (JSON: [{id, data, descricao, valor, registradoPor}])
  // Clientes que já compravam da Agostini ANTES do CRM existir (o pedido do
  // Tiago em 10/09/2026: ele quer lançar vendas de peças de reposição pra
  // esses clientes, mas eles nunca tiveram um card/lead no board). O botão
  // "+ Novo Pós-venda" (ver NovoPosVendaModal.jsx) cria o item direto como
  // "Fechado" (pulando o funil todo) e marca esta caixinha automaticamente.
  // Único uso desse marcador: excluir esses itens das métricas de RITMO do
  // funil na aba Gerencial (Novos leads / Qualificados / desempenho por
  // vendedor) — como o item é criado hoje no monday, sem isso ele contaria
  // como "novo lead hoje", o que não é verdade (é um cliente antigo sendo
  // cadastrado retroativamente). Continua aparecendo normalmente em tudo o
  // resto (Kanban "Fechado", aba Pós-venda) — o Tiago confirmou que não tem
  // problema misturar esses com os fechamentos reais do funil ali.
  clienteHistorico: 'boolean_mm72ka6c', // Cliente Anterior ao CRM — checkbox
};
...
export const DATE_FIELDS = [
  ...
  { value: 'dataEntregaEfetiva', label: 'Data de entrega efetiva' },
];
...
// Estágios do Kanban de Pós-venda (aba Pós-venda) — ciclo de vida do
// cliente depois do FECHAMENTO da venda, separado do funil de vendas acima.
// Só se aplica a leads em "Fechado" (base instalada) e só existe a partir
// do handoff (02/09/2026); leads fechados antes disso não têm
// `estagioPosVenda` preenchido e entram como "Aguardando Entrega" por
// padrão na tela (ver PosVenda.jsx) até alguém arrastar o card.
//
// "Aguardando Entrega" (não "Entregue") é o estágio inicial de propósito —
// ajustado em 04/09/2026 depois do Tiago notar que "Fechado" no CRM é a
// venda assinada, não a máquina na mão do cliente (produção/entrega ainda
// rolam depois disso, às vezes por semanas — ver boards PEDIDOS/PRODUÇÃO).
// O handoff continua acontecendo no fechamento (o pós-venda já se
// apresenta e começa a conversa), mas só vira "Entregue" de verdade quando
// alguém arrasta o card pra lá — e essa transição carimba a data real de
// entrega (ver POS_VENDA_STAGE_DATE_COLUMNS), do mesmo jeito que o funil de
// vendas já carimba data em cada mudança de estágio relevante.
export const POS_VENDA_STAGES = [
  { value: 'Aguardando Entrega', color: '#fdab3d', textColor: '#3a2a05' },
  { value: 'Entregue', color: '#007eb5', textColor: '#ffffff' },
  { value: 'Em Acompanhamento', color: '#9d50dd', textColor: '#ffffff' },
  { value: 'Renovação/Upsell', color: '#9cd326', textColor: '#233900' },
  { value: 'Fidelizado', color: '#037f4c', textColor: '#ffffff' },
];

export const POS_VENDA_STAGE_VALUES = POS_VENDA_STAGES.map((s) => s.value);

// Mapa estágio pós-venda → coluna de data carimbada automaticamente com a
// data de hoje sempre que o card ENTRA nesse estágio (mesmo princípio do
// STAGE_DATE_COLUMNS do funil de vendas, ver acima) — ver PATCH
// /api/items/[id].
export const POS_VENDA_STAGE_DATE_COLUMNS = {
  Entregue: 'dataEntregaEfetiva',
};
