...
// Renomeia o item — diferente de todo o resto do formulário, o "nome" não é
// uma coluna comum do board (não existe em COLUMNS), é o título nativo do
// item no monday.com. `change_multiple_column_values` não altera isso; é
// preciso `change_simple_column_value` com column_id especial "name"
// (confirmado testando direto na API antes de implementar, mesmo padrão já
// usado nas outras colunas deste arquivo). Bug real corrigido em 31/08/2026:
// o formulário do lead nunca teve um campo pra editar isto, então um nome
// capturado errado (ex: pela extensão do WhatsApp, "luizmoreiraandre430" em
// vez de "Luiz Moreira André") não tinha como ser corrigido por ninguém.
export async function renameItem(boardId, itemId, name) {
  const query = `
  ...
  `;
  const data = await mondayRequest(query, { boardId, itemId, value: name });
...
export async function deleteItem(itemId) {
  ...
  `;
  const data = await mondayRequest(query, { itemId });
