...
export async function findNewFileAssetId(itemId, columnId, existingIds) {
  const raw = await fetchItemFileColumnRaw(itemId, columnId);
  const current = (raw.files || []).filter((f) => f.fileType === 'ASSET' && f.assetId);
  const fresh = current.find((f) => !existingIds.has(String(f.assetId)));
  return fresh ? String(fresh.assetId) : null;
}
