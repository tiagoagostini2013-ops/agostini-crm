...
function mapUser(item) {
  ...
  // Vínculo com a pessoa real do monday.com (coluna "people", 1 pessoa) — usado
  // para filtrar a lista de "responsável" no CRM só para quem está em vendas.
  const mondayRaw = c[USER_COLUMNS.mondayUserId]?.value;
  const mondayUserId =
    mondayRaw && Array.isArray(mondayRaw.personsAndTeams) && mondayRaw.personsAndTeams[0]
      ? String(mondayRaw.personsAndTeams[0].id)
      : null;
...
export async function createAppUser(name, senhaHash, { admin = false, mondayUserId = null } = {}) {
  ...
  if (mondayUserId) {
    columnValues[USER_COLUMNS.mondayUserId] = { personsAndTeams: [{ id: Number(mondayUserId), kind: 'person' }] };
  }
...
export async function updateAppUser(itemId, fields) {
  ...
  if (fields.mondayUserId !== undefined) {
    columnValues[USER_COLUMNS.mondayUserId] = fields.mondayUserId
      ? { personsAndTeams: [{ id: Number(fields.mondayUserId), kind: 'person' }] }
      : { personsAndTeams: [] };
  }
